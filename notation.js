'use strict';
// Small, dependency-free SVG score. Accidentals are explicit, with no key signature.
// Each page is an unbarred practice phrase; durations match beats per note.
const ScaleNotation=(()=>{
  ...
  function render(){
    ...
    const svg=el('svg',{viewBox:`0 ${top} 390 ${end-top}`,role:'img','aria-label':`${treble?'Treble':'Bass'} clef. Notes ${start+1}–${start+visible.length}: ${visible.map(n=>n.name).join(', ')}. ${beats} beats per note.`});
    ...
    visible.forEach((n,i)=>{
      const index=start+i,x=65+i*47,y=positions[i],active=index===current;
      ...
      // Every pitch carries an accidental, including naturals: no state to infer across pages.
      group.append(el('text',{x:x-13,y:y+5,'text-anchor':'end','font-size':18,'font-family':'serif',fill:color},{'-2':'♭♭','-1':'♭','0':'♮','1':'♯','2':'𝄪'}[n.accidental]));
    ...
    $('score-page').textContent=`${page+1} / ${Math.max(1,Math.ceil(notes.length/size))} · ${treble?'Treble':'Bass'} clef`;
