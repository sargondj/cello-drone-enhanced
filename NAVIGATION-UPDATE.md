...
- Scales tab: scale guide, shared metronome controls, and an optional root drone. Volume sliders have since been removed; output stays at 100%.
...
Apply all six existing app source files plus navigation.js, notation.js and compact.css. index.html loads all required files. Keep existing favicons and unrelated repository changes. Merge device-tested fixes if the repository has evolved since this package's Stage 3 baseline.

## Checks
Guide, pitch and tuner tests pass. New tests/navigation.cjs checks panel visibility, shared-control movement, cleanup calls, keyboard behavior. Run all four tests and node --check navigation.js before publication.
